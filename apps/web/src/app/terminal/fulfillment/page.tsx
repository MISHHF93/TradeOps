  ...
  return (
    <section>
      <header className="terminal-header">
        <div>
          <p className="pill">Stage view · Sell → Source → Fulfill</p>
          <h1>Fulfillment</h1>
          <p className="lede">
            Cases and orders awaiting sourcing or shipment. Same Commerce Cases as Process — not a
            separate silo.
          </p>
        </div>
      ...
      <article className="panel wide" style={{ marginBottom: 16 }}>
        ...
        {cases.length === 0 ? (
          <p className="meta">
            No Commerce Cases waiting for sales or fulfillment. After publish and orders, cases appear
            here when you sync Process.
          </p>
        ) : (
      ...
      <article className="panel wide">
        ...
        {!orders.ok ? <p className="form-error">{orders.error}</p> : null}
        {orderRows.length === 0 ? (
          <p className="meta">
            No customer orders yet. Fixture path: ingest orders from the Orders page after an active
            listing exists.
          </p>
        ) : (
          <table className="compact">
