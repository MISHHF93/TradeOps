...
export const PRODUCTION_CONNECTORS: ProductionConnectorDescriptor[] = [
  ...
  // Local COS stack — Tavily + Cohere alias used by live-feed-registry / operator runtime
  C({
    id: 'tavily-search',
    provider: 'Tavily',
  ...
  // AI providers — Cohere is the production code-first runtime; others optional adapters
  C({
    id: 'cohere',
    provider: 'Cohere',
    displayName: 'Cohere (primary AI runtime)',
    category: 'ai',
    domain: 'ai_runtime',
    authMethod: 'api_key',
    apiVersion: 'v2',
    ...
    technicalCapabilities: ['chat', 'embeddings', 'rerank', 'tool_use', 'structured_output'],
    webhookTopics: [],
    pollingStrategy: 'none',
    syncIntervalSeconds: 0,
  }),
  C({
    id: 'openai',
    provider: 'OpenAI',
    displayName: 'OpenAI (optional adapter / search)',
    category: 'ai',
    domain: 'ai_runtime',
    authMethod: 'api_key',
    apiVersion: 'v1',
    ...
    scopes: ['chat'],
    businessCapabilities: ['generate_executive_insights', 'optimize_product_content'],
    technicalCapabilities: ['chat', 'embeddings', 'web_search'],
    webhookTopics: [],
    pollingStrategy: 'none',
    syncIntervalSeconds: 0,
  }),
  ...
  C({
    id: 'google-gemini',
    provider: 'Google',
    displayName: 'Google Gemini (optional adapter)',
    category: 'ai',
    domain: 'ai_runtime',
    authMethod: 'api_key',
    apiVersion: 'v1',
  ...
  C({
    id: 'xai',
    provider: 'xAI',
    displayName: 'xAI Grok (optional adapter)',
    category: 'ai',
    domain: 'ai_runtime',
    authMethod: 'api_key',
    apiVersion: 'v1',
...
export function resolveCredentialStatus(
  ...
  // Cohere accepts CO_API_KEY alias (cohere-ai and cohere ids)
  if (
    (connector.id === 'cohere-ai' || connector.id === 'cohere') &&
...
export const CAPABILITY_PROVIDER_MAP: Record<string, string[]> = {
  ...
  optimize_product_content: ['cohere-ai', 'cohere'],
  compare_suppliers: ['fixture-supplier', 'alibaba-open', 'aliexpress-dropshipping'],
  submit_supplier_purchase: ['fixture-supplier'],
  detect_exceptions: ['sentry', 'opentelemetry-collector'],
  estimate_demand: ['google-analytics-4', 'cohere-ai', 'cohere'],
  improve_seo: ['cohere-ai', 'cohere', 'tavily-search'],
